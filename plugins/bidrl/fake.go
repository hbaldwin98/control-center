...
func FakeSite() http.Handler {
	...
	mux.HandleFunc("POST /api/getitems", fakeGetItems)
...
// fakeGetItems is the catalog endpoint: one response carrying every open lot's bid
// state for an auction. The bids are one increment above what ItemData and the pusher
// snapshot report, so a test can tell a catalog refresh apart from the collection that
// preceded it.
func fakeGetItems(w http.ResponseWriter, r *http.Request) {
	if err := r.ParseForm(); err != nil {
		http.Error(w, "form", http.StatusBadRequest)
		return
	}
	auctionID := r.PostFormValue("auction_id")
	ids := make([]string, 0, len(fakeLots))
	for id, lot := range fakeLots {
	...
	for _, id := range ids {
		items = append(items, fakeLots[id].catalog)
	}
	w.Header().Set("Content-Type", "application/json")
	_, _ = io.WriteString(w, `{"total":`+strconv.Itoa(len(items))+`,"page":1,"perpage":500,"total_pages":1,"items":[`+
		strings.Join(items, ",")+`]}`)
}

// fakeCatalogJSON mirrors the production shape: the catalog names the high bidder by id
// only, and reports current_bid as a string on some lots and a number on others.
func fakeCatalogJSON(id, auctionID, title, bid, min, endUnix string, bids int, bidderID string) string {
	return `{"id":"` + id + `","auction_id":"` + auctionID + `","title":"` + title + `",
		"current_bid":"` + bid + `","minimum_bid":` + min + `,"bid_count":"` + strconv.Itoa(bids) + `",
		"high_bidder":"` + bidderID + `","end_time":"` + endUnix + `","time_offset":-7200,
		"current_increment":"1.00","bidding_extended":"0","reserve_met":false}`
}
...
func fakeItemJSON(id, auctionID, title, lotCode, bid, min, endUnix string, bids int, bidderID, bidder string) string {
	return `{
		"item":{
			"id":"` + id + `","auction_id":"` + auctionID + `","title":"` + title + `","lot_number":"` + lotCode + `",
			"description":"<p>` + title + `</p>","current_bid":"` + bid + `","minimum_bid":"` + min + `",
			"high_bidder":"` + bidderID + `","highbidder_username":"` + bidder + `","bid_count":"` + strconv.Itoa(bids) + `","end_time":"` + endUnix + `",
...
func fakePusherJSON(bid, min, endUnix string, bids int, bidderID, bidder string) string {
	return `{"item":{"bid_count":` + strconv.Itoa(bids) + `,"current_bid":` + bid + `,"minimum_bid":` + min + `,
		"high_bidder":"` + bidderID + `","highbidder_username":"` + bidder + `","bidding_extended":false,"end_time":"` + endUnix + `",
		"current_increment":1,"reserve_met":false}}`
}

var fakeLots = map[string]fakeLot{
	"1001": {auctionID: "42", itemData: fakeItemJSON("1001", "42", "Keurig K-Supreme Plus Coffee Maker", "K1001", "15.00", "16.00", "1788396360", 4, "1", "goldwing44"), pusher: fakePusherJSON("17.00", "18.00", "1788396360", 6, "7", "sniper7"), catalog: fakeCatalogJSON("1001", "42", "Keurig K-Supreme Plus Coffee Maker", "18.00", "19.00", "1788396360", 7, "1")},
	"1002": {auctionID: "42", itemData: fakeItemJSON("1002", "42", "Office mesh task chair", "C1002", "8.00", "9.00", "1788396360", 2, "2", "bidder2"), pusher: fakePusherJSON("8.00", "9.00", "1788396360", 2, "2", "bidder2"), catalog: fakeCatalogJSON("1002", "42", "Office mesh task chair", "9.00", "10.00", "1788396360", 3, "2")},
	"1003": {auctionID: "42", itemData: fakeItemJSON("1003", "42", "Aeron-style mesh chair", "C1003", "40.00", "41.00", "1788396360", 6, "3", "bidder3"), pusher: fakePusherJSON("40.00", "41.00", "1788396360", 6, "3", "bidder3"), catalog: fakeCatalogJSON("1003", "42", "Aeron-style mesh chair", "41.00", "42.00", "1788396360", 7, "3")},
	"9001": {auctionID: "99", itemData: fakeItemJSON("9001", "99", "Keurig Mini", "K9001", "12.00", "13.00", "1788396360", 1, "9", "bidder9"), pusher: fakePusherJSON("12.00", "13.00", "1788396360", 1, "9", "bidder9"), catalog: fakeCatalogJSON("9001", "99", "Keurig Mini", "13.00", "14.00", "1788396360", 2, "9")},
}
