...
export function PluginDetail({ plugins }: { plugins: PluginModule[] }) {
      ...
      const states = useSnapshot<PluginState[]>(
            useCallback(
                  (signal) =>
                        api.snapshot<PluginState[]>("/api/admin/plugins", {
                              signal,
                        }),
                  [],
            ),
      ...
      const jobs = useSnapshot<Job[]>(
            useCallback(
                  (signal) =>
                        api.snapshot<Job[]>(
                              `/api/jobs?plugin=${encodeURIComponent(id)}&limit=50`,
                              { signal },
                        ),
                  [id],
            ),
      ...
      const state =
            states.status === "ready"
                  ? states.data.find((p) => p.pluginId === id)
                  : undefined;
...
function SettingsPane({
      ...
      const hasConfig = fieldsFromSchema(state.configSchema).length > 0;
      const hasAI = (state.models ?? []).length > 0;
      const hasEvents = (state.events ?? []).length > 0;

      return (
            <>
                  <nav
                        className="cc-jumpnav"
                        aria-label="Plugin settings sections"
                  >
                        {hasAI ? <a href="#plugin-ai">AI</a> : null}
                        {hasEvents ? <a href="#plugin-events">Events</a> : null}
                        <a href="#plugin-spend">Spend</a>
                        <a href="#plugin-budget">Budget</a>
                        {hasConfig ? (
                              <a href="#plugin-config">Configuration</a>
                        ) : null}
                        <a href="#plugin-kill-switch">Kill switch</a>
                  </nav>

                  {hasAI ? (
                        <div id="plugin-ai" className="cc-anchor-section">
                              <PluginAI state={state} onChanged={onSaved} />
                        </div>
                  ) : null}
                  {hasEvents ? (
                        <div id="plugin-events" className="cc-anchor-section">
                              <PluginEvents state={state} />
                        </div>
                  ) : null}
                  <div id="plugin-spend" className="cc-anchor-section">
                        <Card title="Spend">
                              <SpendWindows state={state} />
                        </Card>
                  </div>
                  <div id="plugin-budget" className="cc-anchor-section">
                        <Card title="Budget">
                              <BudgetForm
                                    key={budgetKey(state)}
                                    pluginId={state.pluginId}
                  ...
                  {hasConfig ? (
                        <div id="plugin-config" className="cc-anchor-section">
                              <ConfigCard
                                    state={state}
                                    onSaved={onSaved}
                                    onError={onError}
                  ...
                  <div id="plugin-kill-switch" className="cc-anchor-section">
