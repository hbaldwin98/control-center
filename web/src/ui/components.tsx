...
export function Grid({
  ...
  return <div className={cx("cc-grid", density && `cc-grid--${density}`)}>{children}</div>;
}
...
export function Money({ microUsd, compact }: { microUsd: number; compact?: boolean }) {
...
export function LiveDot({
  ...
  return <span className={`cc-dot cc-dot--${state}`} role="img" aria-label={label} title={label} />;
}
...
export function Metric({
  ...
  return (
    <div className={cx("cc-metric", tone !== "neutral" && `cc-metric--${tone}`)}>
...
export function Meter({
  ...
  return (
    <div
      ...
      style={{ "--meter-used": `${used * 100}%`, "--meter-held": `${held * 100}%` } as CSSProperties}
    >
      ...
      {values.map((v, i) => {
        ...
        return (
          <rect
            ...
            height={h}
            rx={slot * 0.2}
            className={cx("cc-spark__bar", v === 0 && "cc-spark__bar--empty")}
          />
        );
      })}
    </svg>
